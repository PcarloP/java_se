...
public class FindByAnno extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    String annoParam = request.getParameter("annoNascita");
	    int annoNascita = 0;

	    try {
	        annoNascita = Integer.parseInt(annoParam);
	    } catch (NumberFormatException e) {
	        request.setAttribute("messaggio", "Anno di nascita non valido: inserisci un numero.");
		...
		request.setAttribute("tipoRicerca", "Anno di nascita: " + annoNascita);

		if (persone != null && !persone.isEmpty()) {
		    request.setAttribute("persone", persone);
		} else {
		    request.setAttribute("messaggio", "Nessuna persona trovata per l'anno: " + annoNascita);
		}
