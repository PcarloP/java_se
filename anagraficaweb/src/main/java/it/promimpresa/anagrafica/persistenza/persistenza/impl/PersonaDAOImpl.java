package it.promimpresa.anagrafica.persistenza.persistenza.impl;
...
import it.promimpresa.anagrafica.persistenza.DBUtil;
import it.promimpresa.anagrafica.persistenza.DataSource;
import it.promimpresa.anagrafica.persistenza.PersonaDAO;
...
public class PersonaDAOImpl implements PersonaDAO {
	...
	// metodo che accetta il parametro annoNascita di tipo String 
	...
	// metodo che accetta il parametro annoNascita di tipo int 
	public List<Persona> findByAnno(int annoNascita) throws DAOException {
	    List<Persona> persone = new ArrayList<>();
	    String SQL = "SELECT * FROM persona WHERE year(data_nascita) = ?";
	    System.out.println(SQL);
	    Connection connection = null;
	    ...
	    try {
	        connection = DataSource.getConnection();
	        statement = connection.prepareStatement(SQL);
	        statement.setInt(1, annoNascita);  // usa setInt
	        resultSet = statement.executeQuery();
	        while (resultSet.next()) {
	            persona = new Persona(resultSet.getInt(1), resultSet.getString(2), resultSet.getString(3),
	                    resultSet.getString(4), resultSet.getTimestamp(5).toLocalDateTime());
